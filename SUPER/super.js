...
// This method is the same as the one above, // Shorthand method. 
  ...
  document.title = 'NEW WEBPAGE'; // This changes the title of the document, referencing the HTML document. 
  ...
  <input placeholder="Cost of order" class="js-cost-input" onkeydown=" handleKeydownEvent(event);
 ">
  <button onclick="amazonShipping()" >Calculate</button>
<!-- Display the total -->
...
<br>
  ...
  const newButton = buttonElement.innerHTML = 'New Button2'; 
  buttonElement.classList.add('cs-new-button')
  
  const appendButton = document.body.appendChild(buttonElement); 

//takes the first button element you use the class attribute: class = js-second-button. Here we use the class selectors. 
  document.querySelector('.cs-new-button').innerHTML= 'Changed the new Button' // This changes the name of the button. 
